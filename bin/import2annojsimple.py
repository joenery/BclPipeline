...
def getChromosomeFiles(sam,tdna_filter=False):
    """
    """
    open_files    = {}
    open_files_id = {}
...
def upload2mysql(host,database,tablename,mysql_user,mysql_password,tdna_filter=False):
    """
    """
...
def filter_all(input_file,window_size=200,min_reads=5):
    ...
    remove_clones = "cat all.aj | uniq -f 5 -s 10 -w53 > all.noclones.aj"
    subprocess.call(remove_clones,shell=True)

    # Use Window Technique to mow through clones.
    with open("all.noclones.filter.aj","w") as output_file:
        with open("all.noclones.aj","r") as input_file:
            """
            """
            print("\tFiltering all.noclones.aj")
...
if __name__ == "__main__":
    ...
    advanced.add_argument("-tf","--tdna-filter",     help = "",                                                                   action="store_true")
    ...
    try:
        ...
        if not skip_to_upload:
            getChromosomeFiles(input_file,tdna_filter=tdna_filter)
        
        upload2mysql(host,database,tablename,mysql_user,mysql_password,tdna_filter=tdna_filter)
